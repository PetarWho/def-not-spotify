...
func CreateNativeAPIRouter(ctx context.Context) *nativeapi.Router {
	...
	tagEditor, err := core.NewTagEditor()
	if err != nil {
		panic(err)
	}
	router := nativeapi.New(dataStore, share, playlistsPlaylists, insights, library, user, maintenance, manager, imageUploadService, tagEditor, modelScanner)
	return router
}
